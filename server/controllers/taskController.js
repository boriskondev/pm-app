...
const taskController = {
  ...
  findAllActive: async (req, res) => {
    try {
      ...
      const allTasks = await Task.find(filter).populate([
        {
          path: "clientId",
  ...
  findOne: async (req, res) => {
    try {
      ...
      const foundTask = await Task.findById(id).populate([
        {
          path: "clientId",
          select: { clientName: 1, _id: 1 },
        },
  ...
  edit: async (req, res) => {
    try {
      const { taskName, clientId, projectId, startDate, endDate, responsible } =
        req.body;
      const id = req.params.id;
