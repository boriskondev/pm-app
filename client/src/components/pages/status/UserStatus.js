...
const UserStatus = ({ match }) => {
  ...
  useEffect(() => {
    fetchWrapper.get(endpoints.TASKS_RESPONSIBLE + `/${id}`).then((data) => {
  ...
  const handleDelete = (id) => {
    fetchWrapper._delete(endpoints.TASKS + `/${id}`).then(() => {
  ...
  const handleComplete = (id) => {
    fetchWrapper.patch(endpoints.TASKS + `/${id}`).then(() => {
      setTasksOfUser(tasksOfUser.filter((task) => task._id !== id));
      setShowModalComplete(false);
    });
  };
      ...
      <section className="user-weekly-info">
        ...
        {tasksOfUser.length === 0 && !isLoading && (
          <p className="no-tasks">
            The user has 0 tasks. You can add tasks{" "}
            <Link>
              <span>here</span>
            </Link>
            .
                    ...
                    <ul className="user-status-icons">
                      {task.createdBy === loggedUser.userId ? (
                        <>
                          ...
                          <li onClick={() => handleShowModalComplete(task._id)}>
                            {icons.complete}
                          </li>
                          <ModalComplete
                          ...
                          <li onClick={(e) => handleShowModalDelete(e)}>
                            {icons.delete}
                          </li>
                          <ModalDelete
