...
const ProjectsStatus = () => {
  ...
  useEffect(() => {
    fetchWrapper.get(endpoints.TASKS).then((data) => {
  ...
  const handlePanelClick = (e, id) => {
    fetchWrapper.get(endpoints.PROJECTS + `/${id}`).then((data) => {
    ...
    return (
      <>
        ...
        <section className="content-wrapper">
          <section className="projects-list">{sidebarData}</section>

          <section className="project-info">
            {projectNotClicked && activeTasks.length > 0 && (
              <div className="message">
                <p>
                  This week <span>{activeTasks.length}</span>{" "}
                  {activeTasks.length === 1 ? "task is" : "tasks are"} waiting
                  for us.
                </p>
