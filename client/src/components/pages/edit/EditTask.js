...
const EditTask = ({ match }) => {
  ...
  const handleClientDropdownClick = (e, clientsOptions) => {
    let clientWithProjectsToEnlist = clientsOptions.filter(
  ...
  const handleResponsibleFieldClick = (e, id) => {
    ...
    if (!peopleResponsible.includes(id)) {
      ...
      setPeopleResponsible(
        peopleResponsible.filter((person) => person._id !== id)
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    fetchWrapper.put(endpoints.TASKS + `/${id}`, taskToUpdate).then(() => {
                  ...
                  {responsibleOptions &&
                    responsibleOptions.map((person) => (
                      <li
                        key={person._id}
                        onClick={(e) =>
                          handleResponsibleFieldClick(e, person._id)
                        }
                      >
                        {person.username}
                      </li>
